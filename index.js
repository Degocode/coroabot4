...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const mod = [ownerNumber,"556899068797@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556899068797@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556899068797@s.whatsapp.net"]//ubah nomor lo
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				case 'menu':
                    putagg = await getBuffer(`https://i.ibb.co/SKCnKL9/800px-Super-Crown-NSMBUDeluxe.png`)
